...
def notify_signal(signal: dict):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━\n"
        f"🪙 <b>{signal['symbol']}</b>\n"
        f"📊 Skor: <b>{signal['score']}</b> | Nedenler: {signal['num_reasons']}\n"
        f"💵 Giriş: <code>{signal['entry_price']:.6f}</code>\n"
        f"🛑 SL: <code>{signal['sl']:.6f}</code>\n"
        f"🎯 TP1: <code>{signal['tp1']:.6f}</code>\n"
        f"🎯 TP2: <code>{signal['tp2']:.6f}</code>\n"
        f"🎯 TP3: <code>{signal['tp3']:.6f}</code>\n"
        f"━━━━━━━━━━━━━━━━\n"
        f"📝 {reasons_str}"
    )
    send(text)
