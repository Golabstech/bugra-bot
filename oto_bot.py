import ccxt
import pandas as pd
import pandas_ta as ta
import requests
import time
import sys
from datetime import datetime

# ==========================================
# ⚙️ AYARLAR VE KONFİGÜRASYON
# ==========================================
API_KEY = 'YOUR_API_KEY_HERE'
SECRET_KEY = 'YOUR_SECRET_KEY_HERE'
TELEGRAM_TOKEN = '8063148867:AAH2UX__oKRPtXGyZWtBEmMJOZOMY1GN3Lc'
CHAT_ID = '6786568689'

# Strateji Ayarları
TIMEFRAME = '1h'           # 1 saatlik grafik
LEVERAGE_MSG = "5x"        # Kaldıraç uyarısı
SCORE_THRESHOLD = 70       # Minimum Sinyal Puanı (azaltıldı)
STRONG_SIGNAL_THRESHOLD = 90  # Güçlü sinyal eşiği
MAX_SIGNALS_PER_HOUR = 10   # Saat başı maks sinyal
MIN_VOLUME_USD = 5_000_000  # Minimum hacim
SCAN_COIN_COUNT = 100       # Taranacak coin sayısı (50-100 arası)

# ==========================================
# 🔌 BORSA BAĞLANTISI
# ==========================================
try:
    exchange_config = {
        'enableRateLimit': True,
        'options': {'defaultType': 'future'}
    }
    
    # API Anahtarları varsayılan değilse ekle, aksi halde Public Mode
    if API_KEY != 'YOUR_API_KEY_HERE':
        exchange_config['apiKey'] = API_KEY
        exchange_config['secret'] = SECRET_KEY
    else:
        print("ℹ️ API Anahtarı girilmedi. Bot 'Public Mod'da çalışacak (Sadece izleme).")

    exchange = ccxt.binance(exchange_config)
except Exception as e:
    print(f"Bağlantı Hatası: {e}")
    sys.exit()

# ==========================================
# 🛠️ YARDIMCI FONKSİYONLAR
# ==========================================

def send_telegram_message(message):
    """Telegram üzerinden mesaj gönderir."""
    if TELEGRAM_TOKEN == 'YOUR_TELEGRAM_BOT_TOKEN':
        print(f"TELEGRAM: {message}")
        return

    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    payload = {
        "chat_id": CHAT_ID,
        "text": message,
        "parse_mode": "HTML"
    }
    try:
        requests.post(url, json=payload, timeout=10)
    except Exception as e:
        print(f"Telegram Hatası: {e}")

# ==========================================
# 📊 BINANCE FUTURES TRADING DATA API
# ==========================================

def get_funding_rate(symbol):
    """Binance Futures Funding Rate verisini çeker."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/fapi/v1/fundingRate?symbol={clean_symbol}&limit=1"
        response = requests.get(url, timeout=5).json()
        if response and isinstance(response, list) and len(response) > 0:
            return float(response[0]['fundingRate'])
        return 0.0
    except:
        return 0.0

def get_open_interest(symbol):
    """Binance Futures Open Interest verisini çeker."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/fapi/v1/openInterest?symbol={clean_symbol}"
        response = requests.get(url, timeout=5).json()
        if response and 'openInterest' in response:
            return float(response['openInterest'])
        return 0.0
    except:
        return 0.0

def get_open_interest_hist(symbol):
    """Binance Open Interest değişim verisini çeker (son 4 saat)."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/futures/data/openInterestHist?symbol={clean_symbol}&period=1h&limit=4"
        response = requests.get(url, timeout=5).json()
        if response and isinstance(response, list) and len(response) >= 2:
            current_oi = float(response[-1]['sumOpenInterest'])
            prev_oi = float(response[0]['sumOpenInterest'])
            if prev_oi > 0:
                return ((current_oi - prev_oi) / prev_oi) * 100  # % değişim
        return 0.0
    except:
        return 0.0

def get_taker_buy_sell_ratio(symbol):
    """Taker alım/satım oranını çeker - Piyasa emirleri."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/futures/data/takerlongshortRatio?symbol={clean_symbol}&period=1h&limit=1"
        response = requests.get(url, timeout=5).json()
        if response and isinstance(response, list) and len(response) > 0:
            return float(response[0]['buySellRatio'])
        return 1.0
    except:
        return 1.0

def get_binance_whale_sentiment(symbol):
    """Binance Top Trader Long/Short Ratio verisini çeker."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/futures/data/topLongShortAccountRatio?symbol={clean_symbol}&period=1h&limit=1"
        response = requests.get(url, timeout=5).json()
        if response and isinstance(response, list):
            return float(response[0]['longShortRatio'])
        return 1.0
    except:
        return 1.0

def get_global_long_short_ratio(symbol):
    """Tüm kullanıcıların Long/Short oranını çeker (Genel sentiment)."""
    try:
        clean_symbol = symbol.replace('/', '').split(':')[0]
        url = f"https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol={clean_symbol}&period=1h&limit=1"
        response = requests.get(url, timeout=5).json()
        if response and isinstance(response, list):
            return float(response[0]['longShortRatio'])
        return 1.0
    except:
        return 1.0

def calculate_risk_reward(entry, stop, direction):
    """Risk/Reward oranına göre TP'leri hesaplar."""
    risk = abs(entry - stop)
    if direction == "LONG":
        tp1 = entry + (risk * 1.5)  # 1:1.5 R:R
        tp2 = entry + (risk * 2.5)  # 1:2.5 R:R
        tp3 = entry + (risk * 4)    # 1:4 R:R
    else:
        tp1 = entry - (risk * 1.5)
        tp2 = entry - (risk * 2.5)
        tp3 = entry - (risk * 4)
    return tp1, tp2, tp3

# ==========================================
# 🧠 ANALİZ MOTORU (Sadeleştirilmiş Güçlü İndikatörler)
# ==========================================

def analyze_symbol(symbol, rank):
    """
    Sadeleştirilmiş Profesyonel Analiz Motoru
    En Güçlü 5 İndikatör + Binance Trading Data
    Maksimum Puan: 150
    """
    try:
        # Veri Çekme (retry mekanizması ile)
        ohlcv = None
        for attempt in range(3):
            try:
                ohlcv = exchange.fetch_ohlcv(symbol, TIMEFRAME, limit=150)
                break
            except Exception as e:
                if attempt < 2:
                    time.sleep(1)
                    continue
                else:
                    return None
        
        if not ohlcv or len(ohlcv) < 100:
            return None

        df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
        df.set_index('timestamp', inplace=True)
        
        # ═══════════════════════════════════════════════════════════════
        # SADECE EN GÜÇLÜ İNDİKATÖRLER
        # ═══════════════════════════════════════════════════════════════
        
        # 1. EMA (9, 21, 50) - Trend Belirleme
        df['ema9'] = ta.ema(df['close'], length=9)
        df['ema21'] = ta.ema(df['close'], length=21)
        df['ema50'] = ta.ema(df['close'], length=50)
        
        # 2. RSI (14) - Momentum
        df['rsi'] = ta.rsi(df['close'], length=14)
        
        # 3. MACD - Trend Momentum
        macd = ta.macd(df['close'])
        if macd is not None:
            df['macd'] = macd.iloc[:, 0]
            df['macd_signal'] = macd.iloc[:, 1]
            df['macd_hist'] = macd.iloc[:, 2]
        else:
            df['macd'] = df['macd_signal'] = df['macd_hist'] = 0
        
        # 4. ADX + DI (Trend Gücü)
        adx_data = ta.adx(df['high'], df['low'], df['close'], length=14)
        if adx_data is not None:
            df['adx'] = adx_data.iloc[:, 0]
            df['di_plus'] = adx_data.iloc[:, 1]
            df['di_minus'] = adx_data.iloc[:, 2]
        else:
            df['adx'] = df['di_plus'] = df['di_minus'] = 0
        
        # 5. ATR (Stop Loss için)
        df['atr'] = ta.atr(df['high'], df['low'], df['close'], length=14)
        
        # 6. Volume MA
        df['vol_ma'] = ta.sma(df['volume'], length=20)
        
        # NaN değerleri doldur (deprecated method yerine ffill kullan)
        df = df.ffill().fillna(0)
        
        # Son veriler
        last = df.iloc[-1]
        prev = df.iloc[-2]
        current_price = float(last['close'])
        
        # ═══════════════════════════════════════════════════════════════
        # BINANCE TRADING DATA (YENİ ENTEGRASYON)
        # ═══════════════════════════════════════════════════════════════
        funding_rate = get_funding_rate(symbol)
        oi_change = get_open_interest_hist(symbol)
        taker_ratio = get_taker_buy_sell_ratio(symbol)
        whale_ratio = get_binance_whale_sentiment(symbol)
        global_ls_ratio = get_global_long_short_ratio(symbol)
        
        # Mum karakteristiği
        is_green = last['close'] > last['open']
        is_red = last['close'] < last['open']
        
        # ═══════════════════════════════════════════════════════════════
        # SADELEŞTİRİLMİŞ PUANLAMA (Maks 150 puan)
        # ═══════════════════════════════════════════════════════════════
        long_score = 0
        short_score = 0
        long_reasons = []
        short_reasons = []
        
        # --- 1. ADX + DI (Trend Gücü) - Maks: 30p ---
        adx = float(last['adx']) if pd.notna(last['adx']) else 0
        di_plus = float(last['di_plus']) if pd.notna(last['di_plus']) else 0
        di_minus = float(last['di_minus']) if pd.notna(last['di_minus']) else 0
        
        if adx > 25:
            if di_plus > di_minus:
                long_score += 30
                long_reasons.append(f"📈 ADX Güçlü Trend ({adx:.0f}) + DI+>DI-: +30p")
            else:
                short_score += 30
                short_reasons.append(f"📉 ADX Güçlü Trend ({adx:.0f}) + DI->DI+: +30p")
        elif adx > 20:
            if di_plus > di_minus:
                long_score += 15
                long_reasons.append(f"📈 ADX Orta Trend ({adx:.0f}): +15p")
            else:
                short_score += 15
                short_reasons.append(f"📉 ADX Orta Trend ({adx:.0f}): +15p")
        
        # --- 2. EMA Dizilimi - Maks: 25p ---
        ema9 = float(last['ema9']) if pd.notna(last['ema9']) else current_price
        ema21 = float(last['ema21']) if pd.notna(last['ema21']) else current_price
        ema50 = float(last['ema50']) if pd.notna(last['ema50']) else current_price
        
        if ema9 > ema21 > ema50:
            long_score += 25
            long_reasons.append("✅ EMA Bullish (9>21>50): +25p")
        elif ema9 < ema21 < ema50:
            short_score += 25
            short_reasons.append("✅ EMA Bearish (9<21<50): +25p")
        elif current_price > ema21:
            long_score += 10
            long_reasons.append("📊 Fiyat > EMA21: +10p")
        elif current_price < ema21:
            short_score += 10
            short_reasons.append("📊 Fiyat < EMA21: +10p")
        
        # --- 3. RSI - Maks: 25p ---
        rsi = float(last['rsi']) if pd.notna(last['rsi']) else 50
        
        if rsi < 30:
            long_score += 25
            long_reasons.append(f"🔥 RSI Aşırı Satım ({rsi:.0f}): +25p")
        elif rsi < 40:
            long_score += 15
            long_reasons.append(f"📊 RSI Düşük ({rsi:.0f}): +15p")
        elif rsi > 70:
            short_score += 25
            short_reasons.append(f"🔥 RSI Aşırı Alım ({rsi:.0f}): +25p")
        elif rsi > 60:
            short_score += 15
            short_reasons.append(f"📊 RSI Yüksek ({rsi:.0f}): +15p")
        
        # --- 4. MACD Sinyal - Maks: 20p ---
        macd_hist = float(last['macd_hist']) if pd.notna(last['macd_hist']) else 0
        prev_macd_hist = float(prev['macd_hist']) if pd.notna(prev['macd_hist']) else 0
        macd_val = float(last['macd']) if pd.notna(last['macd']) else 0
        macd_sig = float(last['macd_signal']) if pd.notna(last['macd_signal']) else 0
        
        # MACD crossover kontrolü
        if macd_val > macd_sig and macd_hist > prev_macd_hist:
            long_score += 20
            long_reasons.append("📈 MACD Bullish Cross: +20p")
        elif macd_val < macd_sig and macd_hist < prev_macd_hist:
            short_score += 20
            short_reasons.append("📉 MACD Bearish Cross: +20p")
        
        # ═══════════════════════════════════════════════════════════════
        # BINANCE TRADING DATA PUANLAMASI - Maks: 50p
        # ═══════════════════════════════════════════════════════════════
        
        # Funding Rate Analizi (-0.01 ile +0.01 arası normal)
        if funding_rate < -0.0005:  # Negatif funding = Short'lar ödüyor
            long_score += 15
            long_reasons.append(f"💰 Negatif Funding ({funding_rate*100:.3f}%): +15p")
        elif funding_rate > 0.001:  # Yüksek pozitif = Long'lar ödüyor
            short_score += 15
            short_reasons.append(f"💰 Yüksek Funding ({funding_rate*100:.3f}%): +15p")
        
        # Open Interest Değişimi
        if oi_change > 5:  # OI artıyor
            if is_green:
                long_score += 10
                long_reasons.append(f"📊 OI Artış + Yeşil ({oi_change:.1f}%): +10p")
            else:
                short_score += 10
                short_reasons.append(f"📊 OI Artış + Kırmızı ({oi_change:.1f}%): +10p")
        elif oi_change < -5:  # OI azalıyor = Pozisyon kapatma
            if is_green:
                long_score += 5
                long_reasons.append(f"📊 Short Squeeze olası ({oi_change:.1f}%): +5p")
        
        # Taker Buy/Sell Oranı
        if taker_ratio > 1.1:  # Alıcı baskısı
            long_score += 10
            long_reasons.append(f"🔥 Taker Alım Baskısı ({taker_ratio:.2f}): +10p")
        elif taker_ratio < 0.9:  # Satıcı baskısı
            short_score += 10
            short_reasons.append(f"🔥 Taker Satış Baskısı ({taker_ratio:.2f}): +10p")
        
        # Balina L/S Oranı
        if whale_ratio < 0.85:
            short_score += 15
            short_reasons.append(f"🐋 Balinalar SHORT ({whale_ratio:.2f}): +15p")
        elif whale_ratio > 1.15:
            long_score += 15
            long_reasons.append(f"🐋 Balinalar LONG ({whale_ratio:.2f}): +15p")
        
        # ═══════════════════════════════════════════════════════════════
        # KARAR VE SEVİYE HESAPLAMA
        # ═══════════════════════════════════════════════════════════════
        
        atr = float(last['atr']) if pd.notna(last['atr']) else current_price * 0.02
        final_score = max(long_score, short_score)
        direction = "NEUTRAL"
        stop_loss = 0
        tp1 = tp2 = tp3 = 0
        
        if short_score >= SCORE_THRESHOLD and short_score > long_score:
            direction = "SHORT"
            final_score = short_score
            stop_loss = current_price + (atr * 1.5)
            tp1, tp2, tp3 = calculate_risk_reward(current_price, stop_loss, "SHORT")
            
        elif long_score >= SCORE_THRESHOLD and long_score > short_score:
            direction = "LONG"
            final_score = long_score
            stop_loss = current_price - (atr * 1.5)
            tp1, tp2, tp3 = calculate_risk_reward(current_price, stop_loss, "LONG")
        
        if direction == "NEUTRAL":
            return None  # Sinyal yoksa atla
            
        return {
            'symbol': symbol,
            'direction': direction,
            'score': final_score,
            'long_score': long_score,
            'short_score': short_score,
            'price': current_price,
            'stop': stop_loss,
            'tp1': tp1,
            'tp2': tp2,
            'tp3': tp3,
            'rank': rank,
            'whale': whale_ratio,
            'global_ls': global_ls_ratio,
            'funding': funding_rate,
            'oi_change': oi_change,
            'taker_ratio': taker_ratio,
            'adx': adx,
            'rsi': rsi,
            'long_reasons': long_reasons,
            'short_reasons': short_reasons
        }

    except Exception as e:
        print(f"      ❌ HATA ({symbol}): {str(e)}")
        return None

# ==========================================
# 🚀 ANA DÖNGÜ VE TARAMA
# ==========================================

def get_symbols_by_volume():
    """Tüm Futures çiftlerini hacme göre sıralar."""
    try:
        tickers = exchange.fetch_tickers()
        pairs = []
        for s, d in tickers.items():
            # Binance Futures formatı: BTC/USDT:USDT veya BTC/USDT
            if '/USDT' in s:
                vol = float(d.get('quoteVolume', 0) or 0)
                if vol >= MIN_VOLUME_USD:
                    pairs.append({'symbol': s, 'volume': vol})
        
        # Hacme göre azalan sırala
        pairs.sort(key=lambda x: x['volume'], reverse=True)
        print(f"   📊 Toplam {len(pairs)} adet Futures çifti bulundu (Hacim > {MIN_VOLUME_USD/1e6:.1f}M$).")
        return [p['symbol'] for p in pairs]
    except Exception as e:
        print(f"Ticker Hatası: {e}")
        return []

def main():
    print(f"🤖 BOT AKTİF | Timeframe: {TIMEFRAME} | Hedef Puan: {SCORE_THRESHOLD}")
    send_telegram_message(f"🤖 <b>Bot Başlatıldı!</b>\n⏱ Timeframe: {TIMEFRAME}\n🎯 Tarama: Top {SCAN_COIN_COUNT} Coin\n📊 Strateji: Binance Trading Data + Teknik Analiz")

    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M')}] Tarama Başlıyor...")
            all_symbols = get_symbols_by_volume()
            
            signals = []
            analyzed_count = 0
            
            # 50-100 arasındaki coinleri tara
            scan_end = min(SCAN_COIN_COUNT, len(all_symbols))
            batch = all_symbols[:scan_end]
            
            print(f"   🔍 İlk {scan_end} coin taranıyor...")

            for i, symbol in enumerate(batch):
                rank = i + 1
                try:
                    result = analyze_symbol(symbol, rank)
                    analyzed_count += 1
                    
                    if result:
                        print(f"      ✅ {symbol:<14} | {result['direction']} | Puan: {result['score']}")
                        signals.append(result)
                        
                    if len(signals) >= MAX_SIGNALS_PER_HOUR:
                        break
                        
                except Exception as err:
                    pass
                    
                # Rate limit koruma (her 10 coinde bir bekle)
                if (i + 1) % 10 == 0:
                    time.sleep(1)
            
            print(f"   📊 Toplam {analyzed_count} coin analiz edildi, {len(signals)} sinyal bulundu.")
            
            # Puanı en yüksek olanları öne al
            signals.sort(key=lambda x: x['score'], reverse=True)
            top_signals = signals[:MAX_SIGNALS_PER_HOUR]
            
            if top_signals:
                for s in top_signals:
                    icon = "🟢" if s['direction'] == "LONG" else "🔻"
                    strength = "💪 GÜÇLÜ" if s['score'] >= STRONG_SIGNAL_THRESHOLD else "📊 Normal"
                    
                    # Puan detayları
                    reasons = s.get('long_reasons', []) if s['direction'] == "LONG" else s.get('short_reasons', [])
                    reasons_text = "\n".join([f"  • {r}" for r in reasons[:5]])
                    
                    # Risk hesaplama
                    risk_pct = abs(s['price'] - s['stop']) / s['price'] * 100
                    rr_ratio = abs(s['tp2'] - s['price']) / abs(s['price'] - s['stop'])
                    
                    msg = (
                        f"{icon} <b>#{s['symbol'].split('/')[0]} - {s['direction']} ({LEVERAGE_MSG})</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"⭐ <b>PUAN:</b> {s['score']}/150 ({strength})\n"
                        f"🏆 <b>Hacim Sırası:</b> #{s['rank']}\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"📥 <b>GİRİŞ:</b> ${s['price']:.4f}\n"
                        f"🛑 <b>STOP LOSS:</b> ${s['stop']:.4f} ({risk_pct:.1f}%)\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"🎯 <b>TP1:</b> ${s['tp1']:.4f} (R:R 1:1.5)\n"
                        f"🎯 <b>TP2:</b> ${s['tp2']:.4f} (R:R 1:2.5)\n"
                        f"🎯 <b>TP3:</b> ${s['tp3']:.4f} (R:R 1:4)\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"<b>📊 Binance Trading Data:</b>\n"
                        f"  🐋 Balina L/S: {s['whale']:.2f}\n"
                        f"  📈 Global L/S: {s['global_ls']:.2f}\n"
                        f"  💰 Funding: {s['funding']*100:.4f}%\n"
                        f"  📊 OI Değişim: {s['oi_change']:.1f}%\n"
                        f"  🔥 Taker B/S: {s['taker_ratio']:.2f}\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"<b>📈 Teknik:</b> ADX: {s['adx']:.0f} | RSI: {s['rsi']:.0f}\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"<b>Sinyal Sebepleri:</b>\n{reasons_text}\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"⚠️ <i>DYOR - Bu finansal tavsiye değildir</i>"
                    )
                    send_telegram_message(msg)
                    print(f"      📤 Telegram'a gönderildi: {s['symbol']}")
                    time.sleep(1)
            else:
                print("   ❌ Bu turda uygun sinyal bulunamadı.")
            
            print(f"⏳ 2 Dakika bekleniyor...")
            time.sleep(120)

        except Exception as e:
            print(f"Genel Hata: {e}")
            time.sleep(60)

if __name__ == "__main__":
    main()
